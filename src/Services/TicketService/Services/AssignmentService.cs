...
public class AssignmentService : IAssignmentService
{
    ...
    public AssignmentService(ITicketRepository ticketRepository, UserManagementClient userManagementClient)
    {
    ...
    public async Task<Guid?> FindLeastBusyTechnicianAsync( Guid? excludedTechnicianId = null)
    {
        ...
        foreach (var technician in technicians.Where(item => item.Id != excludedTechnicianId).OrderBy(item => item.Id))
        {
